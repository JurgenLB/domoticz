...
std::string base64_encode_buf(unsigned char const* bytes_to_encode, unsigned int in_len) {
  std::string ret;
  int i = 0;
...
std::string base64url_decode(std::string const& encoded_string) {
  ...
  while (in_len-- && ( encoded_string[in_] != '=') && is_base64url(encoded_string[in_])) {
    ...
    if (i ==4) {
      for (i = 0; i <4; i++)
        char_array_4[i] = (unsigned char)base64url_chars.find(char_array_4[i]);
  ...
  if (i) {
    ...
    for (j = 0; j <4; j++)
      char_array_4[j] = (unsigned char)base64url_chars.find(char_array_4[j]);
