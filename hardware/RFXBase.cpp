...
bool CRFXBase::CheckValidRFXData(const uint8_t *pData)
{
	...
	_log.Log(LOG_STATUS, "Data received: length=%u, type=0x%02X, payload=%s", static_cast<unsigned int>(pLen), static_cast<unsigned int>(pType), ToHexString(pData, pLen + 1).c_str());
