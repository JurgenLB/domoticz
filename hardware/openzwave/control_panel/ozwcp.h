...
class MyNode
{
	...
	int32 getValueCount();
	static MyValue *lookup(const std::string &id);
	...
	static void addRemoved(uint8 node)
	{
		removed.push_back(node);
	}
	static uint32 getRemovedCount()
	{
		return (uint32)removed.size();
	}
...
class COpenZWaveControlPanel
{
	...
	void setAdminState(bool st)
	{
		adminstate = st;
	}
	std::string getAdminFunction()
	{
		return adminfun;
	}
	void setAdminFunction(std::string msg)
	{
		adminfun = msg;
	}
	...
	void setAdminMessage(std::string msg)
	{
		adminmsg = msg;
	}
