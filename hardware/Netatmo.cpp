	...
	, m_password(CURLEncode::URLDecode(password))
{
	...
	m_bPollHomeStatus = find_scopes(); //"thermostat_RW","camera_RWA","presence_RWA","carbonmonoxidedetector_R","smokedetector_R","magellan_RW","bubendorff_RW","smarther_RW","mx_RW","mhs1_RW","station_R","camerapro_RWA"
			...
			if (!module["id"].empty())
			{
				...
				if (connected)
				{
					...
					if ((type == "NATherm1") || (type == "NRV"))
					{
						...
						if (setModeSwitch)
						{
							Debug(DEBUG_HARDWARE, "Schedule mode %s %s %s", m_Room_mode[roomNetatmoID].c_str(), roomNetatmoID.c_str(), setpoint_mode_str.c_str());
