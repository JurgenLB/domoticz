...
// Other possible scopes But not currently in use are;
//  read_magellan write_magellan read_bubendorff write_bubendorff read_mx write_mx read_mhs1 write_mhs1 read_doorbell access_doorbell
#define NETATMO_REDIRECT_URI "http://localhost/netatmo"
// https://api.netatmo.com/oauth2/authorize?client_id=<CLIENT_ID>&redirect_uri=http://localhost/netatmo&state=teststate&scope=read_station%20read_smarther%20write_smarther%20read_thermostat%20write_thermostat%20read_camera%20write_camera%20access_camera%20read_doorbell%20access_doorbell%20read_presence%20write_presence%20access_presence%20read_homecoach%20read_carbonmonoxidedetector%20read_smokedetector
...
void SaveUchar2Disk(std::map<unsigned char, std::string> &m, std::string filename)
{
        int count = 0;
        std::ofstream fOut;
        fOut.open(filename);
...
void SaveUint8_t2Disk(std::map<uint8_t, std::string> &m, std::string filename)
{
        int count = 0;
        std::ofstream fOut;
        fOut.open(filename);
...
void SaveMapint2Disk(std::map<int, int> &m, std::string filename)
{
        int count = 0;
        std::ofstream fOut;
        fOut.open(filename);
...
void SaveDict2Disk(std::map<int, std::string> &m, std::string filename)
{
        int count = 0;
        std::ofstream fOut;
        fOut.open(filename);
...
void SaveMap2Disk(std::map<std::string, std::string> &m, std::string filename)
{
        int count = 0;
        std::ofstream fOut;
        fOut.open(filename);
...
//        std::string httpUrl(NETATMO_API_URI + "oauth2/token?")
	...
	ret = ParseJSon(sResult, root);
        //Debug(DEBUG_HARDWARE, "root = %s ", root.toStyledString().c_str());
...
bool CNetatmo::WriteToHardware(const char* pdata, const unsigned char /*length*/)
{
	...
	if (packettype == pTypeLighting2)
	{
		...
		bool bIsOn = (pCmd->LIGHTING2.cmnd == light2_sOn);
                ...
                std::string name = "";
                uint64_t ulId1 = (ulong) id1; // PRIu64
                int idx;
                int iUsed;
                bool bValid;
...
//                                        m_DeviceID[Hardware_int] = DeviceRowIdx;            // idx
//                                        m_thermostatModuleID[uid] = module_type;            // mac-adres
...
//                                        m_ModuleNames[module_id] = name;                    // Device Name
//                                        m_RoomIDs[module_id] = roomNetatmoID;               // Room Netatmo ID
//                                        m_DeviceHomeID[roomNetatmoID] = homeID;             // Home_ID
//
//                Debug(DEBUG_HARDWARE, "Netatmo mac %s", mac_ID.c_str());
...
//        m_RetransSourceId = pdata;  //std::to_string(pdata );
//        _log.Debug(DEBUG_HARDWARE, "debug_pdata %s ", m_RetransSourceId.c_str());
...
//                _log.Debug(DEBUG_HARDWARE, "sSwitchTypeSelector %s ", sSwitchTypeSelector);
//		return false;
...
//		return true;
	...
	//
//	if ((int)(pCmd->LIGHTING2.unitcode) == 10)

		//Recast raw data to get switch specific data
//		const _tGeneralSwitch* xcmd = reinterpret_cast<const _tGeneralSwitch*>(pdata);
//                int len = xcmd->len;             //uint8_t len;
//                int type = xcmd->devType;           //uint8_t type;
                //uint8_t devType = xcmd->devType;
                //int len = xcmd->packetlength;
                //int type = xcmd->packettype;
//                int subtype = xcmd->subtype;     //uint8_t subtype;
//		int ID = xcmd->id;               //int32_t id - switch ID
                //int ID1 = xcmd->id1;
                //int ID3 = xcmd->id2;
                //int ID2 = xcmd->id3;
                //int ID4 = xcmd->id4;
//                int unitcode = xcmd->unitcode;   //int32_t unitcode - Unitcode
//                int cmnd = xcmd->cmnd;         //uint8_t cmnd -
//                int level = xcmd->level;         //uint8_t level - Level selected on the switch
                //int filler = xcmd->filler;
                //int rssi = xcmd -> SignalLevel;
                //int battery_level = xcmd -> BatteryLevel;
                //std::string roomNetatmoID = xcmd -> roomNetatmoID;
//                int seqnbr = xcmd->seqnbr ;      //uint8_t seqnbr;
//                int reserved_int = xcmd->reserved_int;     //int32_t reserved_int;   #619
//                float reserved_float = xcmd->reserved_float; //float reserved_float;
                //
//                std::stringstream sw_id;
//                sw_id << std::hex << ID;
//                std::string strhex = sw_id.str();

//                _log.Debug(DEBUG_HARDWARE, "Netatmo THERMOSTAT selector");
//                //Debug(DEBUG_HARDWARE, "Netatmo devType  = %d", devType);
//                //Debug(DEBUG_HARDWARE, "Netatmo len  = %d", len);
//                //Debug(DEBUG_HARDWARE, "Netatmo type  = %d", type);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo subtype  = %d", subtype);
//                //Debug(DEBUG_HARDWARE, "Netatmo ID1  = %d", ID1);
//                //Debug(DEBUG_HARDWARE, "Netatmo ID2  = %d", ID2);
//                //Debug(DEBUG_HARDWARE, "Netatmo ID3  = %d", ID3);
//                //Debug(DEBUG_HARDWARE, "Netatmo ID4  = %d", ID4);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo unitcode  = %d", unitcode);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo cmd  = %d", cmnd);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo level  = %d", level);
//                //Debug(DEBUG_HARDWARE, "Netatmo filler  = %d", filler);

//                Debug(DEBUG_HARDWARE, "Netatmo len      = %d", len);
//                Debug(DEBUG_HARDWARE, "Netatmo type     = %d", type);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo subtype  = %d", subtype);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID     = %d  - %s", ID, strhex.c_str());
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID1    = %d", (pCmd->LIGHTING2.id1));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID2    = %d", (pCmd->LIGHTING2.id2));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID3    = %d", (pCmd->LIGHTING2.id3));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID4    = %d", (pCmd->LIGHTING2.id4));
//                //Debug(DEBUG_HARDWARE, "Netatmo ID     = %d", (pCmd->LIGHTING2.id));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo unitcode = %d", unitcode);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo level = %d", level);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo seqnbr = %d", seqnbr);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo reserved_int   = %d", reserved_int);
//                _log.Debug(DEBUG_HARDWARE, "Netatmo reserved_float = %f", reserved_float);
...
//		int mode = (level / 10) - 1;
//		unsigned long therm_id = (pCmd->LIGHTING2.id1 << 24) + (pCmd->LIGHTING2.id2 << 16) + (pCmd->LIGHTING2.id3 << 8) + pCmd->LIGHTING2.id4;
//                _log.Debug(DEBUG_HARDWARE, "Netatmo len      = %d", (pCmd->LIGHTING2.packetlength));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo type     = %d", (pCmd->LIGHTING2.packettype));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo subtype  = %d", (pCmd->LIGHTING2.subtype));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID     = %d  - ", (ID));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID1    = %d", (pCmd->LIGHTING2.id1));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID2    = %d", (pCmd->LIGHTING2.id2));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID3    = %d", (pCmd->LIGHTING2.id3));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID4    = %d", (pCmd->LIGHTING2.id4));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID4    = %d  IDX ", (pCmd->LIGHTING2.id4));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo command  = %d", (pCmd->LIGHTING2.cmnd));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo unitcode = %d", (pCmd->LIGHTING2.unitcode));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo level    = %d", (pCmd->LIGHTING2.level));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo filler   = %d", (pCmd->LIGHTING2.filler));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo reserved_int   = %d", (pCmd->LIGHTING2.rssi));
//                Debug(DEBUG_HARDWARE, "Netatmo seqnbr++ = %d", (pCmd->LIGHTING2.seqnbr++));
//                Debug(DEBUG_HARDWARE, "Netatmo reserved_float = %d", (pCmd->LIGHTING2.reserved_float));


//        //This is the selector switch for the Floodlight
//        const _tGeneralSwitch* xcmd = reinterpret_cast<const _tGeneralSwitch*>(pdata);
//        unsigned long _id = (pCmd->LIGHTING2.id1 << 24) + (pCmd->LIGHTING2.id2 << 16) + (pCmd->LIGHTING2.id3 << 8) + pCmd->LIGHTING2.id4;
//
//                //Recast raw data to get switch specific data
//
//                const _tGeneralSwitch* xcmd = reinterpret_cast<const _tGeneralSwitch*>(pdata);
...
//                Debug(DEBUG_HARDWARE, "Netatmo reserved_float = %d", (pCmd->LIGHTING2.reserved_float));
//
//                _log.Debug(DEBUG_HARDWARE, "Netatmo len      = %d", (xcmd->len));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo type     = %d", (xcmd->type));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo subtype  = %d", (xcmd->subtype));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID     = %d  - %s", ID, strhex.c_str());
//                _log.Debug(DEBUG_HARDWARE, "Netatmo ID1    = %d", (xcmd->id));
//                Debug(DEBUG_HARDWARE, "Netatmo ID1    = %d", (xcmd->id1));
//                Debug(DEBUG_HARDWARE, "Netatmo ID2    = %d", (xcmd->id2));
//                Debug(DEBUG_HARDWARE, "Netatmo ID3    = %d", (xcmd->id3));
//                Debug(DEBUG_HARDWARE, "Netatmo ID4    = %d", (xcmd->id4));
//                Debug(DEBUG_HARDWARE, "Netatmo ID4    = %d", (xcmd->id4));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo unitcode = %d", (xcmd->unitcode));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo level    = %d", (xcmd->level));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo seqnbr   = %d", (xcmd->seqnbr));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo reserved_int   = %d", (xcmd->reserved_int));
//                _log.Debug(DEBUG_HARDWARE, "Netatmo reserved_float = %f", (xcmd->reserved_float));
//
...
bool CNetatmo::SetProgramState(const int uid, const int newState)
{
	...
	if (!m_isLogged == true)               //Double ifnot m_isLogged is true
	{
        ...
        Debug(DEBUG_HARDWARE, "Netatmo SetProgram id = %d - %08X %d", uid, uid, newState);
...
//                                        m_thermostatModuleID[uid] = module_type;            // mac-adres
//                                        m_thermostats[DeviceRowIdx] = roomNetatmoID;        // Room Netatmo ID
//                                        m_ModuleIDs[Hardware_int] = crcId;                  // crcId
//                                        m_DeviceModuleID[uid] = module_id;                  // mac-adres
//                                        m_ModuleNames[module_id] = name;                    // Device Name
//                                        m_RoomIDs[module_id] = roomNetatmoID;               // Room Netatmo ID
//                                        m_DeviceHomeID[roomNetatmoID] = homeID;             // Home_ID
//
        ...
        Debug(DEBUG_HARDWARE, "Netatmo %s ; %s in Room ID =  %s in Home: %s", name.c_str(), module_id.c_str(), roomNetatmoID.c_str(), Home_id.c_str());
        Debug(DEBUG_HARDWARE, " roomNetatmoID = %s", roomNetatmoID.c_str());
        Debug(DEBUG_HARDWARE, " HomeID = %s", Home_id.c_str());
        //
	if (!m_thermostatModuleID[uid].empty())
	{
                ...
                if (!bRet)
		{
			...
			return false;
                ...
                _data = "home_id=" + Home_id + "&device_types=" + Type + "&";
        ...
        if(!m_LightDeviceID[uid].empty())
	{
		...
		{
		case 0:
			State = "off"; //The Floodlight is currently following its weekly schedule
			break;
		case 1:
			State = "on"; //The Floodlight is currently applying the away temperature
			break;
		case 2:
			State = "auto";
			break;
		default:
			...
			return false;
                ...
                json_data["home"]["modules"]["id"] = module_id;
                json_data["home"]["modules"]["floodlight"] = State;
                std::string extra_data = json_data.toStyledString();
                //std::string extra_data = "{\"home\":{\"id\":\"" + Home_id + "\",\"modules\":[{\"id\":\"" + module_id + "\",\"floodlight\":\"" + State + "\"}]}}" + "&";
                home_data = "";
                Get_Respons_API(NETYPE_SETSTATE, sResult, home_data, bRet, root, extra_data);
		if (!bRet)
                {
...
//void CNetatmo::SetSetpoint(0, (uint8_t)((ID & 0x00FF0000) >> 16), (ID & 0XFF00) >> 8, ID & 0XFF, 1, sp_temp, sName) // const uint8_t ID1, const uint8_t ID2, const uint8_t ID3, const uint8_t ID4, const uint8_t Unit, const float Value, const std::string& defaultname
void CNetatmo::SetSetpoint(unsigned long ID, const float temp)
//void CNetatmo::SetSetpoint(int id, const float temp)
{
	...
	if (!m_isLogged == true)
	{
        ...
        std::string id;
        id = std::to_string(ID);
//        _tNetatmoDevice* pSensor = &m_known_thermotats[ID];
        Json::Value root;
        //const tRBUF* pCmd = reinterpret_cast<const tRBUF*>(nDevice);
        //std::string IID = nDevice.ID;
        //int Icmnd = nDevice.cmnd;         //uint8_t cmnd -
        //int Iunitcode = nDevice.unitcode;   //int32_t unitcode - Unitcode
        //int Ilevel = nDevice.level;         //uint8_t level - Level selected on the switch
        //int rssi = nDevice.SignalLevel;
        //int battery_level = nDevice.BatteryLevel;
        //std::string home_id = nDevice.home_id;
        //std::string module_name = nDevice.ModuleName;
        //std::string bridge_name = nDevice.StationName;
        //std::string room_Netatmo_id = nDevice.roomNetatmoID;
        //std::string MAC_id = nDevice.MAC;
        //
        Debug(DEBUG_HARDWARE, "DevID = %08X", ID);                //correct
        //
//                                        m_thermostat_IDX[DeviceRowIdx] =     
//                                        m_thermostatDeviceID[Hardware_int] = 
...
//                                        m_DeviceHomeID[roomNetatmoID] = homeID;             // Home_ID
        ...
        std::string module_id = m_DeviceModuleID[ID];             //uid
        std::string name = m_ModuleNames[module_id];
	...
	if (m_energyType != NETYPE_ENERGY)
	{
		// Check if thermostat device is available
                if (m_DeviceModuleID[ID].empty())
		{
			Log(LOG_ERROR, "NetatmoThermostat : No thermostat found in online devices!");
			return;
		}

                home_data = "home_id=" + Home_id + "&room_id=" + roomNetatmoID.c_str() + "&mode=" + Mode  + "&temp=" + std::to_string(temp)  + "&endtime=" + std::to_string(end_time) + "&";
...
bool CNetatmo::SetSchedule(int scheduleId)
{
	...
	std::stringstream bstr;
        std::string sResult;
        Json::Value root;       // root JSON object
        std::string home_data = "home_id=" + m_Home_ID + "&mode=schedule" + "&"; //schedule, away, hg
        bool bRet;              //Parsing status
	if (m_energyType == NETYPE_ENERGY)
	{
...
void CNetatmo::GetHomesDataDetails()
{
	...
	std::string sResult;  // text returned by API
        ...
        bool bRet;            //Parsing status
        Json::Value root;     // root JSON object
        Debug(DEBUG_HARDWARE, "Get HomesData Details ");
        ...
        if (!root["body"]["homes"].empty())
	{
                ...
                for (auto home : root["body"]["homes"])
                {
                        if (!home["id"].empty())
                        {
                                ...
                                Debug(DEBUG_HARDWARE, "Get HomesData ID %s", homeID.c_str());
                                //
                                ...
                                if (!home["rooms"].empty())
                                {
                                        for (auto room : home["rooms"])
                                        {
                                                 ...
                                                 for (auto module_ids : room["module_ids"])
                                                 {
                                                         ...
                                                         //Debug(DEBUG_HARDWARE, "mID %s - room %s - Type %s - Name %s ",module_id.c_str() , roomID.c_str(), m_Types[roomID].c_str(), m_RoomNames[roomID].c_str());
...
void CNetatmo::GetHomeStatusDetails()
{
	...
	std::string home_data;
        ...
        Debug(DEBUG_HARDWARE, "Home Data length %ld ", m_homeid.size());

        for (int i = 0; i < m_homeid.size(); i++)
        {
                home_id = m_homeid[i];
                home_data = "home_id=" + home_id + "&";
	...
	for (auto device : root["body"]["devices"])
                ...
                if (!device["_id"].empty())
		{
			...
			std::string name;
                        ...
                        Debug(DEBUG_HARDWARE, " %lu -  %s " , module_id, id.c_str());
			...
			nDevice.StationName = station_name;
                        ...
                        //m_thermostats[iDevIndex] = nDevice;
                        ...
                        {
                                // 86=bad, 56=good mrf_status =
                                RF_status = (86 - device["wifi_status"].asInt()) / 10;
                                if (RF_status > 10)
                                        RF_status = 10;
                        }
                        if (!device["rf_status"].empty())
                        {
                                RF_status = (86 - device["rf_status"].asInt()) / 10;
                                if (RF_status > 10)
                                        RF_status = 10;
                        }
                        // Homecoach
					...
					for (auto module : device["modules"])
					{
						if (module.isObject())
						{
							...
							std::string mname = module["module_name"].asString();
                                                        ...
                                                        Debug(DEBUG_HARDWARE, " %lu -  %s " , moduleID, mid.c_str());
                                                        m_ModuleIDs[moduleID] = crcId;
							if (mname.empty())
								mname = "unknown" + mid;
							...
							{
								// 90=low, 60=highest
								mrf_status = (90 - module["rf_status"].asInt()) / 10;
								if (mrf_status > 10)
									mrf_status = 10;
							}
							//
...
//        Debug(DEBUG_HARDWARE, "bHave %d : %d : %d : %d : %d : %d : %d : %d", bHaveTemp, bHaveHum, bHaveBaro, bHaveCO2, bHaveRain, bHaveSound, bHaveWind, bHaveSetpoint);
  ...
  uint64_t Hardware_int = convert_mac(Hardware_ID);
  ...
  UpdateValueInt(Hardware_int, str_ID.c_str(), 2, pTypeGeneral, sTypePercentage, '0', 255, '0', sValue.c_str(), module_name, 0, m_Name); // RF Percentage
  //
	...
	if (bHaveSetpoint)
	{
                ...
                //SendSetPointSensor(const uint8_t ID1, const uint8_t ID2, const uint8_t ID3, const uint8_t ID4, const uint8_t Unit, const float Value, const std::string& defaultname)
	...
	if (bHaveRain)
	{
		...
		if (rain < m_OldRainCounter[ID])
			m_RainOffset[ID] += m_OldRainCounter[ID];  //#2091 conversion from 'int' to 'float'
                ...
                for (auto room : mRoot)
                {
                        if (!room["id"].empty())
                        {
                                ...
                                if (!room["therm_setpoint_temperature"].empty())
                                {
                                        ...
                                        //SendSetPointSensor(const uint8_t ID1, const uint8_t ID2, const uint8_t ID3, const uint8_t ID4, const uint8_t Unit, const float Value, const std::string& defaultname)
                                        //SendSetPointSensor((uint8_t)((crcId & 0x00FF0000) >> 16), (roomNetatmoID & 0XFF00) >> 8, roomNetatmoID & 0XFF, room["therm_setpoint_temperature"].asInt(), roomName);
                                ...
                                if (!room["therm_setpoint_mode"].empty())
                                {
                                        ...
                                        //Send Selector Switch(thermostatID, 3, setpoint_mode_i, m_ThermostatName[id] + " - Mode", 15, true, "Off|On|Away|Frost Guard", "", true, m_Name);
                                        Debug(DEBUG_HARDWARE, "Room %s - Mode %s / Temp = %s C Setpoint = %s", roomName.c_str(), setpoint_mode.c_str(), room_temp.c_str(), room_setpoint_str.c_str());
                                        //Send Selector Switch(crcId, 3, setpoint_mode, roomName + " - Mode", 15, true, "Off|On|Away|Frost Guard", "", true, m_Name);
                                        //
		...
		for (auto module : mRoot)
		{
			if (!module["id"].empty())
			{
				std::string module_id = module["id"].asString();
                                ...
                                uint64_t Hardware_int = convert_mac(module_id);
                                //converting ID to char const
				...
				//
                                ...
                                UpdateValueInt(Hardware_int, ID.c_str(), 1, pTypeGeneral, sTypeTextStatus, '0', 255, '0', sValue.c_str(), module_Name, 0, m_Name);
                                //
                                //battery_state
				...
				if (!module["battery_level"].empty())
				{
					...
					float battery_Level = module["battery_level"].asFloat() / 100;
                                        ...
                                        //
                                        UpdateValueInt(Hardware_int, ID.c_str(), 2, pTypeGeneral, sTypeCustom, '0', 255, '0', bat_percentage.c_str(), batName,  '0', m_Name);
				};
				...
				if (!module["rf_strength"].empty())
				{
					...
					if (mrf_percentage != 0)
					{
						std::string pName = " " + moduleName + " - Sig. + Bat. Lvl";
                                                ...
                                                UpdateValueInt(Hardware_int, ID.c_str(), 3, pTypeGeneral, sTypePercentage, mrf_status, batteryLevel, '0', sValue.c_str(), pName,  '0', m_Name);
					}
				};
                                ...
                                {
                                        // 86=bad, 56=good
                                        //Debug(DEBUG_HARDWARE, "Wifi_strength");
                                ...
                                if (!module["bridge"].empty())
                                {
                                        ...
                                        UpdateValueInt(Hardware_int, ID.c_str(), 4, pTypeGeneral, sTypeTextStatus, '0', 255, '0', sValue.c_str(), Module_Name, 0, m_Name);
                                };
                                if (!module["boiler_valve_comfort_boost"].empty())
                                {
                                        ...
                                        //DeviceRowIdx = UpdateValueInt(Hardware_int, ID.c_str(), 0, pTypeGeneralSwitch, sSwitchGeneralSwitch, '0', 255, '0', sValue.c_str(), bName,  bIsActive, m_Name);
                                };
					...
					//SendTextSensor(crcId, 2, batteryLevel, module["status"].asString(), a_Name);
                                        ...
                                        UpdateValueInt(Hardware_int, ID.c_str(), 2, pTypeGeneral, sTypeAlert, '0', 255, '0', sValue.c_str(), a_Name, 0, m_Name);
					bool bIsActive = (module["status"].asString() == "open");
                                        //SendSwitch(crcId, 0, batteryLevel, bIsActive, 0, aName, m_Name, mrf_percentage);
                                        //UpdateValueInt(Hardware_int, module_id.c_str(), 3, pTypeGeneralSwitch, sSwitchGeneralSwitch, mrf_status, batteryLevel, '0', sValue.c_str(), a_Name, bIsActive, m_Name);
					UpdateValueInt(Hardware_int, ID.c_str(), 5, pTypeGeneralSwitch, sSwitchGeneralSwitch, mrf_status, batteryLevel, '0', sValue.c_str(), a_Name, bIsActive, m_Name);
                                ...
                                if (!module["floodlight"].empty())
                                {
                                        ...
                                        std::string LevelActions = "0|1|2";
                                        //nDevice.Selector = Selector;
                                        ...
                                        if (!result.empty())
                                        {
                                                Idx = std::stoi(result[0][0]);
                                                Debug(DEBUG_HARDWARE, "Floodlight name %s - IDX %d  - %s / mode %s", lName.c_str(), Idx,  sValue.c_str(), status.c_str());
                                        };
                                        //Send Selector Switch(const int NodeID, const uint8_t ChildID, const std::string &sValue, const std::string &defaultname,
                                        //                   const int customImage, const bool bDropdown, const std::string &LevelNames,
                                        //                   const std::string &LevelActions, const bool bHideOff, const std::string &userName)
                                        //void Send Selector Switch(int NodeID, uint8_t ChildID, &sValue, &defaultname, customImage, nDropdown, &LevelNames, &LevelActions, bHideOff, &userName);
                                        //Send Selector Switch(crcId, 6, sValue, lName, 11, true, LevelNames, LevelActions, false, m_Name);
...
//                                        m_sql.SetDeviceOptions(DeviceRowIdx, Options);
                                ...
                                if ((type == "NATherm1") || (type == "NRV"))
                                {
                                        ...
                                        //sp_temp = static_cast<int>(room_setpoint);
                                        int sp_temp = stoi(room_setpoint);
                                        //
                                        int uid = crcId;
                                        //
                                        ...
                                        Debug(DEBUG_HARDWARE, "roomNetatmoID %d  %ld -  %s %s in Home; %s ", crcId , mid, module_id.c_str(), roomNetatmoID.c_str(), home_id.c_str());
                                        m_thermostat_IDX[DeviceRowIdx] = Hardware_int;        // Hardware_int
                                        m_DeviceModuleID[uid] = module_id;                    // mac-adres
                                        //m_thermostatDeviceID[Hardware_int] = DeviceRowIdx;  // idx
                                        m_thermostatModuleID[crcId] = module_id;              // mac-adres
                                        ...
                                        //int moduleID = convert_mac(module_id);
                                        std::string sValue;
                                        std::stringstream h;
                                        h << bIsActive;
                                        h >> sValue;
                                        ...
                                        if (type == "NATherm1")
                                        {
                                                ...
                                                //UpdateValueInt(Hardware_int, module_id.c_str(), 1, pTypeGeneralSwitch, sSwitchGeneralSwitch, mrf_status, batteryLevel, bIsActive, sValue.c_str(), aName,  0, m_Name);
                                        }
                                        //
...
//                                        m_thermostat_IDX[DeviceRowIdx] = Hardware_int;      // Hardware_int
//                                        m_DeviceID[Hardware_int] = DeviceRowIdx;            // idx
//                                        m_thermostatModuleID[uid] = module_type;            // Types
//                                        m_DeviceHomeID[roomNetatmoID] = home_id;            // Home_ID
//                                        m_thermostats[DeviceRowIdx] = roomNetatmoID;        // Room Netatmo ID
//                                        m_DeviceModuleID[uid] = module_id;                  // mac-adres
//                                        m_ModuleNames[module_id] = name;                    // Device Name
...
//                                        m_LightDeviceID[uid] = lName
                                        ...
                                        // Save Map_uint64_t_int2Disk(m_ModuleIDs, std::string("./") + "m_ModuleIDs.txt"); //m_ModuleIDs[mID] = crcId;
                                        //SaveDict2Disk(m_ModuleIDs, std::string("./") + "m_ModuleIDs.txt");
                                        SaveMap2Disk(m_ModuleNames, std::string("./") + "m_ModuleNames.txt");
                                        SaveMap2Disk(m_RoomIDs, std::string("./") + "m_RoomIDs.txt");
                                        SaveMap2Disk(m_Types, std::string("./") + "m_Types.txt");
                                        SaveMap2Disk(m_Module_category, std::string("./") + "m_Module_category.txt");
                                        SaveMap2Disk(m_Room_setpoint, std::string("./") + "m_Room_setpoint.txt");
                                        SaveMap2Disk(m_Room_mode, std::string("./") + "m_Room_mode.txt");
                                        SaveMap2Disk(m_Room_Temp, std::string("./") + "m_Room_Temp.txt");
                                        SaveMap2Disk(m_RoomNames, std::string("./") + "m_RoomNames.txt");
                                        SaveMap2Disk(m_ThermostatName, std::string("./") + "m_ThermostatName.txt");
                                        //SaveMap2Disk(m_thermostatModuleID, std::string("./") + "m_thermostatModuleID.txt");
                                        //SaveDict2Disk(m_thermostat_IDX, std::string("./") + "m_thermostat_IDX.txt");
                                        // Save Map_uint64_t_int2Disk(m_thermostatDeviceID, std::string("./") + "m_thermostatDeviceID.txt");
                                        SaveDict2Disk(m_thermostatModuleID, std::string("./") + "m_thermostatModuleID.txt");
                                        // Save Map_uint64_t_int2Disk(m_DeviceID, std::string("./") + "m_DeviceID.txt");
                                        SaveMap2Disk(m_DeviceHomeID, std::string("./") + "m_DeviceHomeID.txt");
                                        SaveDict2Disk(m_thermostats, std::string("./") + "m_thermostats.txt");
                                        // Save Map_int_uint64_t2Disk(m_thermostat_IDX, std::string("./") + "m_thermostat_int.txt");
                                        SaveUchar2Disk(m_DeviceModuleID, std::string("./") + "m_DeviceModuleID.txt");
                                        SaveUint8_t2Disk(m_DeviceModuleID, std::string("./") + "m_DeviceModuleID_8.txt");
                                        //
                                };
        ...
        if (!root["body"]["home"]["events"].empty())
        {
			...
			// Using Textstatus / Alert for now
                        ...
                        if (!events["module_id"].empty())
                        {
                                ...
                                //int crcId = Crc32(0, (const unsigned char*)events_Module_ID.c_str(), events_Module_ID.length());
                                //UpdateValueInt(Hardware_int, ID, 1, pTypeGeneral, sTypeTextStatus, '0', 255, '0', sValue.c_str(), Module_Name, 0, m_Name);
                                ...
                                std::string module_Name = e_Name + " - MAC-adres";
                                UpdateValueInt(Hardware_int, str_id.c_str(), 1, pTypeGeneral, sTypeTextStatus, '0', 255, '0', sValue.c_str(), module_Name, 0, m_Name); //Events
                        };
...
//                                                std::string events_subevents_Vignette = sub_events["vignette"]["expires_at"].asString();
			...
			if (!events["person_id"].empty())
				...
				//UpdateValueInt(Hardware_int, events_Module_ID.c_str(), 6, pTypeGeneral, sTypeAlert, RF_status, 255, nValue, sValue.c_str(), e_Name, 0, m_Name);
                                UpdateValueInt(Hardware_int, str_id.c_str(), 5, pTypeGeneral, sTypeAlert, RF_status, 255, nValue, sValue.c_str(), e_Name, 0, m_Name);
                        };
                        //
                };
        };
